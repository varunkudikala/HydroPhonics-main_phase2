...
boolean Adafruit_FONA::enableGPRS(boolean onoff) 
{
  if (_type == SIM5320A || _type == SIM5320E || _type == SIM7500 || _type == SIM7600) {
    ...
    {
        Serial.println(F("Failed AT+NETOPEN"));
        //return false;
    }
     if (! sendCheckReply(F("AT+HTTPINIT"), ok_reply, 10000))
     {
        Serial.println(F("Failed AT+HTTPINIT"));
        //return false;
     }
#endif        
      }
...
*/
void  Adafruit_FONA_LTE::HTTP_POST_SensorValues(float TempValue,float TDSValue,float PHValue,float ECGData)
{
     char buff[500]={0},retryCount =5;
     bool retVal = true;
     do
     {
      //sprintf(buff,"AT+HTTPPARA=\"URL\",\"http://api.asksensors.com/write/59Ga4lSkI9a7liVsoekPIB6PV0JaUIrR?module1=%d&&module2=%d\"",rand()%100,rand()%100);
      //sprintf(buff,"AT+HTTPPARA=\"URL\",\"http://api.asksensors.com/write/8WhPKreJ8CmZ6mjpZ7qEhySbr4zCtdsm?module1=%f&&module2=%f&&module3=%f&&module4=%f\"\r",TempValue,TDSValue,PHValue,ECGData);
      sprintf(buff,"AT+HTTPPARA=\"URL\",\"http://industrial.ubidots.com/api/v1.6/devices/ECG_Monitor/?token=BBFF-bVGiwqVPxAFKpcxxXvI2PK4mhEkuJl&_method=post&ECG_Variable=%f\"\r",ECGData);
      //Serial.println(buff);
      if (! sendCheckReply(buff, ok_reply, 10000))
      {
        retVal=false;
        //Serial.println(F("Failed AT+HTTPPARA"));
        //delay(100);
        //return false;   
      }
      else
      {
        ...
        {
          retVal=false;
          //Serial.println(F("Failed AT+HTTPACTION"));
          //return false;  
          delay(500);
        }
        else
     ...
     }while((retVal==false)&&(retryCount--));
      //delay(500);
#if 0
     if (! sendCheckReply(F("AT+HTTPTERM"), ok_reply, 10000))
      {
        Serial.println(F("Failed AT+HTTPTERM"));
        //return false;            
      }
      if (! sendCheckReply(F("AT+NETCLOSE"), ok_reply, 10000))
      {
        Serial.println(F("Failed AT+NETCLOSE"));
        //return false;                    
      }
#endif  
}
