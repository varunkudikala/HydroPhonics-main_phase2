...
void DFRobot_ESP_PH_WITH_ADC::phCalibration(byte mode)
{
    ...
    case 2:
        if (enterCalibrationFlag)
        {
            if ((this->_voltage > PH_8_VOLTAGE1) && (this->_voltage < PH_6_VOLTAGE1))
            { // buffer solution:7.0
                Serial.println();
            ...
            else if ((this->_voltage > PH_5_VOLTAGE1) && (this->_voltage < PH_3_VOLTAGE1))
            { //buffer solution:4.0
                Serial.println();
    ...
    case 3: //store calibration value in eeprom
        if (enterCalibrationFlag)
        {
            ...
            {
                if ((this->_voltage > PH_8_VOLTAGE1) && (this->_voltage < PH_5_VOLTAGE1))
                {
                    EEPROM.writeFloat(PHVALUEADDR, this->_neutralVoltage);
                    EEPROM.commit();
                }
    ...
    case 3: //store calibration value in eeprom
        if (enterCalibrationFlag)
        {
            ...
            if (phCalibrationFinish)
            {
                ...
                if ((this->_voltage > PH_VOLTAGE_NEUTRAL_LOW_LIMIT) && (this->_voltage < PH_VOLTAGE_NEUTRAL_HIGH_LIMIT))
                {
                    Serial.print(F(">>>Before EEPROMWrite1"));
                    Serial.print(this->_neutralVoltage);
                    Serial.println(this->_eepromStartAddress);
                    EEPROM.writeFloat(this->_eepromStartAddress, this->_neutralVoltage);
                    EEPROM.commit();
                }
                //buffer solution:4.0
                ...
                {                    
                    Serial.print(F(">>>Before EEPROMWrite2"));
                    Serial.print(this->_acidVoltage);
                    Serial.println(this->_eepromStartAddress);
                    EEPROM.writeFloat(this->_eepromStartAddress + (int)sizeof(float), this->_acidVoltage);
                    EEPROM.commit();
